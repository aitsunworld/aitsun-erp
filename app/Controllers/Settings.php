...
class Settings extends BaseController
{
     public function index()
    {
          ...
          if ($session->has('isLoggedIn')){
                ...
                $Main_item_party_table->where('id',$myid);
                $user=$Main_item_party_table->first();
                ...
                if (usertype($myid)=='customer') {
                 ...
                 $data = [
                     'title' => 'Aitsun ERP-Account Settings',
                     'user'=>$user,
    ...
    public function save_profile($userid=""){
          if ($this->request->getMethod() == 'post') {
                    ...
                    $myem=$Main_item_party_table->where('id',$userid)->first();
                    ...
                    if ($_FILES['user_profile']['size'] < 500000) {
                        if ($myem['email']==$email) {
                             ...
                             $Main_item_party_table->update($userid,$users_data);
                             ...
                             // return redirect()->to(base_url('settings/account_settings'));
                              ...
                              $checkemail=$Main_item_party_table->where('email',$email)->where('deleted',0)->first();
        ...
        if ($session->has('isLoggedIn')) {
          ...
          if (app_status(company($myid))==0) {return redirect()->to(base_url('app_error'));}
              ...
              if (check_permission($myid,'manage_account_setting')==true || usertype($myid)=='admin') {}else{ return redirect()->to(base_url('app_error/permission_denied'));}
                ...
                $data=[
                    'title'=>'Organisation settings | Aitsun ERP',
                    'user'=>$usaerdata,
    ...
    public function financial_years(){
         ...
         if($session->has('isLoggedIn')){
                ...
                $data = [
                    'title' => 'Aitsun ERP- Financial Year',
            ...
            if (current_financial_year('financial_from',company($myid))=='no_financial_years'){
                ...
                if ($finins) {
                    ...
                    $finnewyrdata = [
                        ...
                        'financial_year'=>$insert_id,
                    ];

                    $Main_item_party_table->update($myid,$finnewyrdata);

                    $cash_sale_id=$Main_item_party_table->where('company_id',company($myid))->where('display_name','Cash Sale')->first();

                    $cash_sale_data = [
                        'financial_year'=>$insert_id,
                    ];

                    $Main_item_party_table->update($cash_sale_id['id'],$cash_sale_data);
                        ...
                        if (last_financial_year('id',company($myid))=="not_exist") {
                            ...
                            add_base_accounting_heads(company($myid),activated_year(company($myid)));
                            transfer_accounting_heads(company($myid),last_financial_year('id',company($myid)),$insert_id);
                            $result=1;
        ...
        public function activate_f_year($fyear=""){
            if (!empty($fyear)) {
                ...
                // }
                    ...
                    if ($Main_item_party_table->update($myid,$finyrdata)) {
...
public function preferences(){
            ...
            $session=session();
               ...
               if($session->has('isLoggedIn')){
                   ...
                   $cquery = $CompanySettings->where('company_id', company($myid))->first();
                    ...
                    $data = [
                            'title' => 'Aitsun ERP- Invoice settings',
                            'user' => $user,
                            'c_set' => $cquery
                    ];
    ...
    public function prefixes(){
               ...
               if($session->has('isLoggedIn')){
                ...
                $data = [
                        'title' => 'Aitsun ERP- Prefix settings',
                        'user' => $user,
                        'c_set' => $cquery
                ];
            ...
            $session=session();
             if($session->has('isLoggedIn')){
               ...
               $cquery = $CompanySettings->where('company_id', company($myid))->first();
                ...
                $data = [
                        'title' => 'Aitsun ERP- Payment settings',
                        'user' => $user,
                        'c_set' => $cquery
                ];
     ...
     public function product(){
        ...
        $session=session();
            ...
            $data=[
                'title'=>'Aitsun ERP- Product settings',
                'user'=> $user,
                'c_set' => $cquery,
                'p_units' => $puqry,
                'p_cate' => $pcuqry,
    ...
    public function team(){
             ...
             if($session->has('isLoggedIn')){
                    ...
                    //     foreach(aitsun_get_admins_of_company($ci['id']) as $cid){
                ...
                $data = [
                    'title' => 'Aitsun ERP- Company Members',
                    'user' => $user,
            ...
            if ($session->has('isLoggedIn')) {
              ...
              if (app_status(company($myid))==0) {return redirect()->to(base_url('app_error'));}
                  ...
                  $student_sub_cat=$StudentcategoryModel->where('company_id',company($myid))->where('deleted',0)->where('type','sub')->findAll();
                    ...
                    $data=[
                        'title'=>'Caste Category | Aitsun ERP',
                        'user'=>$usaerdata,
    ...
    public function payment_gateway()
            ...
            $CompanySettings = new CompanySettings();
                ...
                if ($session->has('isLoggedIn')) {
                    ...
                    if (app_status(company($myid))==0) {return redirect()->to(base_url('app_error'));}
                        ...
                        $data=[
                            'title'=>'Payment Gate way | Aitsun ERP',
                            'user'=>$usaerdata,
        ...
        if ($session->has('isLoggedIn')) {
            ...
            if (check_permission($myid,'manage_academic_year')==true || usertype($myid)=='admin') {}else{ return redirect()->to(base_url('app_error/permission_denied'));}
            
            $ac_years=$AcademicYearModel->where('company_id',company($myid))->where('deleted',0)->orderby('id','desc')->findAll();
                
                $data=[
                    'title'=>'Academic Year settings | Aitsun ERP',
                    'user'=>$usaerdata,
    ...
    public function invoice_settings($invoice_type="sales"){
        ...
        public function get_form($invoice_type="sales"){
            ...
            $data=[
                'title'=>'Invoice Settings - Aitsun ERP',
    ...
    public function save_invoice_settings($invoice_type){
        ...
        if ($this->request->getMethod('post')) { 
            ...
            $inv_data=[
                'invoice_page_size'=>strip_tags($this->request->getVar('invoice_page_size')),
                'invoice_orientation'=>strip_tags($this->request->getVar('invoice_orientation')),
                'pdf_scaling'=>strip_tags($this->request->getVar('pdf_scaling')),
