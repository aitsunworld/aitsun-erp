        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb aitsun-breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">
                    <b class="page_heading text-dark">All Transactions</b>
                </li>
            </ol>
        </nav>
...
<div class="toolbar d-flex justify-content-between">
    <div>
        <a href="javascript:void(0);" class="aitsun_table_export text-dark font-size-footer me-2" data-type="excel" data-table="#all_transaction_table" data-filename="All Transactions <?= str_replace(array('From','To','&nbsp;','Today','Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','day'), '', $report_date); ?>"> 
            <span class="my-auto">Excel</span>
        </a>
        <a href="javascript:void(0);" class="aitsun_table_export text-dark font-size-footer me-2" data-type="csv" data-table="#all_transaction_table" data-filename="All Transactions <?= str_replace(array('From','To','&nbsp;','Today','Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','day'), '', $report_date); ?>"> 
            <span class="my-auto">CSV</span>
        </a>
        <a href="javascript:void(0);" class="aitsun_table_export text-dark font-size-footer me-2" data-type="pdf" data-table="#all_transaction_table" data-filename="All Transactions <?= str_replace(array('From','To','&nbsp;','Today','Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','day'), '', $report_date); ?>"> 
        ...
        <a href="javascript:void(0);" class="aitsun_table_quick_search text-dark font-size-footer me-2" data-table="#all_transaction_table"> 
        ...
        <div class="filter_bar">
            ...
            <form method="get" class="d-flex">
               ...
               <input type="date" name="from" class="form-control form-control-sm filter-control" title="From" placeholder="">
                ...
                <a class=" btn-outline-dark btn btn-sm" href="<?= base_url('special_reports/all-transactions') ?>"><?= langg(get_setting(company($user['id']),'language'),'Clear'); ?></a>
...
<div class="sub_main_page_content ">
    <div class="aitsun-row pb-5">
        <div class="aitsun_table w-100 pt-0">
            ...
            <table id="all_transaction_table" class="erp_table sortable">
                 <thead>
                    <tr>
                    ...
                    <tr> 
                        ...
                        <th class="sorticon"><?= langg(get_setting(company($user['id']),'language'),'Balance'); ?></th>  
                    </tr>
                 </thead>

                   <tbody>
                <?php  $data_count=0; ?>
                    <?php $debit_sum=0; $credit_sum=0; $total_sum=0; foreach ($day_book_data as $db) { $data_count++; ?>
                        <tr <?php if ($db['deleted']!=0): ?>style="background: #ff000040; opacity: 0.7;" data-tableexport-display="none" <?php endif ?>>
                            ...
                            <?php if ($db['invoice_id']>0): ?>
                                ...
                                (
                                    ...
                                    <?php else: ?>
                                        ...
                                        <?php if ($db['bill_type']=='sales' || $db['bill_type']=='proforma_invoice' || $db['bill_type']=='purchase' || $db['bill_type']=='sales_return' || $db['bill_type']=='purchase_return'): ?>
                          ...
                          <td>
                                <?php if ($db['bill_type']=='sales' || $db['bill_type']=='proforma_invoice' || $db['bill_type']=='purchase_return' || $db['bill_type']=='purchase' || $db['bill_type']=='sales_return'): ?>
                                  ...
                                  <?= get_group_data($db['account_name'],'display_name'); ?>
                                  <?php endif ?>
                            ...
                            <?php if (invoice_data($db['invoice_id'],'invoice_type')=='challan'): ?>
                                ...
                                <?php elseif ($db['bill_type']=='sales' || $db['bill_type']=='proforma_invoice' || $db['bill_type']=='purchase' || $db['bill_type']=='sales_return' || $db['bill_type']=='purchase_return'): ?>
                                 ...
                                 <?php elseif($db['bill_type']=='proforma_invoice'): ?>
                          ...
                          <td>
                              ...
                              <?php if ($db['bill_type']=='sales' || $db['bill_type']=='proforma_invoice' || $db['bill_type']=='purchase_return' || $db['bill_type']=='purchase' || $db['bill_type']=='sales_return'): ?>
                                   ...
                                   <?php elseif($db['bill_type']=='expense'): ?>
                                    Expense
                                  <?php elseif($db['bill_type']=='receipt'): ?>
                                  Income
                                  <?php endif ?>
                          ...
                          <td>
                               ...
                               <?php if ($db['bill_type']=='expense' || $db['bill_type']=='receipt'): ?>
                             <?= currency_symbol(company($user['id'])); ?><?= aitsun_round($db['amount'],get_setting(company($user['id']),'round_of_value')); ?>
                              <?php else: ?>
                               <?= currency_symbol(company($user['id'])); ?> <?= aitsun_round(invoice_data($db['invoice_id'],'total'),get_setting(company($user['id']),'round_of_value')); ?>
                              <?php endif ?>

                              <?php if ($db['deleted']==0): ?>
                                <?php $total_main = (float)$db['amount']+(float)invoice_data($db['invoice_id'],'total'); 
                                    $total_sum+=$total_main; ?>
                               <?php endif ?>
                          ...
                          <td class="text-right" <?php if ($db['deleted']!=0): ?>style="text-decoration: line-through;"<?php endif ?>>
                            <?php if ($db['bill_type']=='receipt' || $db['bill_type']=='sales' || $db['bill_type']=='proforma_invoice' || $db['bill_type']=='purchase_return'): ?>
                               <?= currency_symbol(company($user['id'])); ?> <?= aitsun_round($db['amount'],get_setting(company($user['id']),'round_of_value')); ?>
                               <?php if ($db['deleted']==0): ?>
                ...
                <tfoot>
                    <tr>
                        <th colspan="4"><strong><?= langg(get_setting(company($user['id']),'language'),'Total'); ?></strong></th>
                        <th class="text-right">
                          <strong><?= currency_symbol(company($user['id'])); ?> <?= aitsun_round($total_sum,get_setting(company($user['id']),'round_of_value')); ?></strong>
                        </th>
                        <th class="text-right">
