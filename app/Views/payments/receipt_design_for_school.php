 ...
 <table border="1" cellspacing="0" cellpadding="5" style="width:100%;font-size: 14px;" class="challan_sheet_tb">
	<tr>
		<td colspan="3">
			...
			<table cellspacing="0" cellpadding="0" border="0">
				<tr>
					<td> 
						<h3 style="margin:0;"><b><?= organisation_name($invoice['company_id']); ?></b></h3>
	...
	<tr bgcolor="#0e0e0e">
		<td colspan="3" style="text-align:center;" >
			<h4 style="margin:0; color: white;"><?php if ($bill_type=='expense'): ?> 
				PAYMENT
			<?php else: ?>
				RECEIPT
			<?php endif; ?></h4>
		...
		<td colspan="3">
			...
			<table cellspacing="0" cellpadding="0" border="0" style="font-size: 14px;">
				...
				<tr>
					<td >
					<div>
						<?php if ($bill_type=='expense'): ?> 
								Payment
							<?php else: ?>
								Receipt
							<?php endif; ?> No : <?= get_setting($invoice['company_id'],'payment_prefix'); ?>
                <?= serial_cash($invoice['company_id'],$payment_id); ?><br>
					...
					<td style="text-align:left; vertical-align: baseline; width: 40%;">
						<div class="my-auto mr-5"> 
							<?php if ($bill_type=='expense'): ?> 
								Payment
							<?php else: ?>
								Receipt
							<?php endif; ?> Date: <span style=""><?= get_date_format($datetime,'d M Y') ?></span><br>
							Challan Date: <span style=""><?= get_date_format($invoice['invoice_date'],'d M Y') ?></span><br>
	...
	<tr>
		<td colspan="2" style="padding:3px 3px 3px 4px;">Amount received </td>
		<td style="text-align:right; padding:3px 3px 3px 4px;"><span style="font-family: DejaVu Sans;"><?= currency_symbol($invoice['company_id']) ?></span> <?= number_format($amount,get_setting(company($user['id']),'round_of_value'),'.',''); ?></td>
	</tr>
	...
	<?php if ($invoice['paid_amount']!=0): ?>
		...
		<tr>
			<td colspan="3" style="padding:3px 3px 3px 4px;">Amount in Words: <span style="text-transform: capitalize; "><?= numberTowords(aitsun_round($amount,get_setting(company($user['id']),'round_of_value')),$invoice['company_id']); ?></span></td>
		</tr>
	<?php endif ?>
