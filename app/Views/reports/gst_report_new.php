...
<div class="toolbar d-flex justify-content-between">
    <div>
        ...
        <a id="exportExcel" class="text-dark font-size-footer me-2">Excel</a>
    ...
    <div class="aitsun-row "> 
        ...
        <div class="aitsun_table table-responsive col-12 w-100 pt-0 pb-5">
            ...
            <table id="gst_table" class="erp_table sortable">
             <thead>
                <tr>
                    <th colspan="<?= 10+(count(all_budpayina_taxes(company($user['id'])))*2) ?>" class="text-start" >
                ...
                <tr>
                    ...
                    <?php $ist=0; foreach (all_budpayina_taxes(company($user['id'])) as $hta):  $ist++;?>
                        <?php ${"isav".$ist}=false;   ${"isav".$ist}=true;?>
            ...
            <?php $grand_sub_total=0;$grand_ta_total=0;$grand_total=0; $grand_round_total=0; $grand_rounded_total=0; foreach ($gst_reports as $db) { $data_count++; ?> 
              <tbody>
                  <tr class="text-center">
                        ...
                        <?php $ai=0; $tai=0; $ttaxx=0; foreach (all_budpayina_taxes(company($user['id'])) as $ta): $ttaxx+=0; ?>
                            ...
                            <td>  
                                 ...
                                 <?= aitsun_round($db[str_replace(['%',' @ ','.'], '_', $ta).'taxable_amount'],get_setting(company($user['id']),'round_of_value')) ?>
                                 <?php  ${"taxvalvar".$tai}+=$db[str_replace(['%',' @ ','.'], '_', $ta).'taxable_amount']; ?>
                                 
                            </td>

                            <td>
                                <?= aitsun_round($db[str_replace(['%',' @ ','.'], '_', $ta).'tax_amount'],get_setting(company($user['id']),'round_of_value')) ?>
                                <?php  ${"taxvar".$ai}+=$db[str_replace(['%',' @ ','.'], '_', $ta).'tax_amount']; ?>
                            </td>
                            <?php endif ?>
                        <?php endforeach ?> 
...
<script type="text/javascript">
    $(document).ready(function () { 
        ...
        $("#exportExcel").click(function () {
            let table = document.getElementById("gst_table"); // Your table ID
            let workbook = XLSX.utils.table_to_book(table, {sheet: "Sheet1"});
            XLSX.writeFile(workbook, "GST Report <?= str_replace(array('From','To','&nbsp;','Today','Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','day'), '', $report_date); ?>.xlsx");
