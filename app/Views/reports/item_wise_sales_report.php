    ...
    <div class="right_bar d-flex justify-content-between w-100">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb aitsun-breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">
                    <b class="page_heading text-dark">Item wise sales report</b>
                </li>
            </ol>
        </nav>
                ...
                <!-- FILTER -->
                  <form method="get" class="d-flex">
                    <?= csrf_field(); ?>
                      ...
                      <a class=" btn-outline-dark btn btn-sm" href="<?= base_url('item_wise_sales_report') ?>"><?= langg(get_setting(company($user['id']),'language'),'Clear'); ?></a>
        ...
        <div class="aitsun_table col-12 w-100 pt-0 pb-5">
            ...
            <table id="profit_table" class="erp_table sortable">
             <thead>
              ...
              <tbody>
                <?php  $data_count=0; $total_amount=0;?>

                <?php foreach ($invoice_items_array as $ii): $data_count++; $total_amount+=aitsun_round($ii['sum_amount'],get_setting(company($user['id']),'round_of_value')); ?> 
                <tr>
                    <td>
                    ...
                    <td><?= $ii['product'] ?></td>
                    <td><?= round($ii['sum_quantity'],3) ?></td>
                    <td><?= aitsun_round($ii['sum_amount'],get_setting(company($user['id']),'round_of_value')) ?></td> 
                </tr>
                <?php endforeach ?>
              ...
              <tfoot>
                    <tr>
                        ...
                        <th class="text-right" style="border:1px solid #00000012;"><?= aitsun_round($total_amount,get_setting(company($user['id']),'round_of_value')); ?></th>
