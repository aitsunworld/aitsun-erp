...
<div class="outer">
      ...
      <thead>
        ...
        <?php endif ?> 
          ...
          <th colspan="<?= $first_row_fixed ?>" style="border: 0;">
            <div class="text-center p-3" style="margin-top:115px;">
  ...
  <tr style="border: 1px solid;">
    ...
    <td style="border-left: 1px solid;" colspan="3" class="m-2 col-4">
      <?php if (get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'invoice_num')==1): ?>
        <p class="m-2 mb-0 mt-0">
          ...
          <?php else: ?>
            <b>Invoice:</b>
          <?php endif ?>
          <span style="    margin-left: 79px;"><?= inventory_prefix($invoice_data['company_id'],$invoice_data['invoice_type']); ?><?= $invoice_data['serial_no']; ?></span>
      ...
      <?php if (get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'show_date')==1): ?>
         <p class="m-2 mb-0 mt-0"><b>Date:</b> <span style="margin-left: 96px;"><?= get_date_format($invoice_data['invoice_date'],'d M Y'); ?></span></p>
      <?php endif ?>
      ...
      <p class="m-2 mb-0 mt-0">
        <?php if (get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'mode_of_payment')==1): ?>
            <?php if (trim(mode_of_payment($invoice_data['company_id'],$invoice_data['id']))!=''): ?>
              ...
              <b><?= langg(get_setting($invoice_data['company_id'],'language'),'Mode of Payment'); ?> :</b>
              <span >
      ...
      <p class="m-2 mb-0 mt-0">
        <?php if (!empty($cmp['gstin_vat_no'])): ?>
          <?php if (get_company_data($invoice_data['company_id'],'country')=='India'):?>
            <b>GSTIN: </b><span style="margin-left: 77px;"><?= $cmp['gstin_vat_no']; ?></span>
          <?php else: ?>
            <b>VATTIN:</b> <span style="margin-left: 77px;"><?= $cmp['gstin_vat_no']; ?></span>
          <?php endif ?> 
        <?php endif ?>
      </p>
      <p class="m-2 mb-0 mt-0"> 
      <?php if (get_company_data($invoice_data['company_id'],'country')=='Oman'):?>
            <?php if (get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'show_tax_tin_num')==1): ?>
              ...
              <b> TAX CARD: </b><?= get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'taxnumber'); ?> , <b>TIN:</b> <?= get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'tinnumber'); ?>
              <?php endif ?>
  ...
  <tr style="border: 1px solid;">
    ...
    <td style="border-right:1px solid black;padding: 3px 0; text-align: center;width: 50%;"><b><?= langg(get_setting($invoice_data['company_id'],'language'),'Description of Goods'); ?></b>
    </td>
    ...
    <?php if (get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'show_tax')==1): ?>
      <td style="border-right:1px solid black; padding: 3px 0; text-align: center;" class="<?php if ($invoice_data['invoice_type']=='sales_delivery_note'){echo 'dl-none';} ?>">
        <b>
          <?php if (get_company_data($invoice_data['company_id'],'country')=='India'):?>
  ...
  <?php $taxcount=0; $slno=0; $current = 0; $count = count(invoice_items_array($invoice_data['id'])); foreach (invoice_items_array($invoice_data['id']) as $ii): $slno++; $current++;?> 
    ...
    <tr style="border-right: 1px solid;border-left: 1px solid;border-top: 1px solid;border-bottom: 0;" >
      <td class="px-2" colspan="5" style="text-align: end;"><b>Total</b></td>
      ...
      <?php $ttot=0;foreach (all_taxes_of_invoice($invoice_data['company_id'],$invoice_data['id']) as $tds): ?>
        <?php if (!empty($tds['tax_name'])): ?>
          <tr style="border-right: 1px solid;border-left: 1px solid;border-bottom: 0;">
            <td class="px-2" colspan="5" style="text-align: end;"><?= $tds['tax_name']; ?> :</td>
            <td class="px-2" style="border-left: 1px solid;text-align: end;">
      ...
      <?php if (get_company_data($invoice_data['company_id'],'country')!='Oman'):?>
        <tr nobr="true">
          <td class="px-2" colspan="5" style="text-align: end;"><?= langg(get_setting($invoice_data['company_id'],'language'),'Taxable Amount'); ?> :</td>
    ...
    <tr style="border-right: 1px solid;border-left: 1px solid;">
      <td class="px-2" colspan="5" style="text-align: end;"><b>Invoice Amount</b></td>
      <td class="px-2" style="border-left: 1px solid;text-align: end; "> 
  ...
  <tr style="border-top: 1px solid;">
    <th colspan="<?= $first_row_fixed ?>">
        <div class="my-auto" style="width:100%">
          <div class="  <?php if ($invoice_data['invoice_type']=='sales_delivery_note'){echo 'display:none;';} ?>">
            <p class="ms-2" style="margin: 0px;" class="text-capitalize"> <?= currency_symbol2($invoice_data['company_id']); ?> <?= ucwords(numberTowords(aitsun_round($invoice_data['total'],get_setting($invoice_data['company_id'],'round_of_value')),$invoice_data['company_id'])); ?>.
            </p> 
          </div>
        </div>
    </th>
...
<tr style="border: 1px solid;" colspan="<?= $first_row_fixed ?>">
    <td colspan="2">
     <?php if (get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'show_declaration')==1): ?>
         <?php if (!empty(get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'invoice_declaration'))){ ?>
            ...
            <div style="font-size:16px;" class="ms-2 ml-2">
              <b><?= langg(get_setting($invoice_data['company_id'],'language'),'Declaration'); ?> :</b>
            </div>
            <p class="ms-2 ml-2"> <?= get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'invoice_declaration'); ?></p>
    ...
    <td style="border-left: 1px solid;" colspan="4" class="m-2">
       <h5 class="text-end me-2" style="font-size:16px; margin-bottom: 0; margin-top: 12px; color:black; height: 90px;"><b>For, <?= my_company_name($invoice_data['company_id']); ?></b>
          </h5>
          <?php if (get_invoicesetting($invoice_data['company_id'],$invoice_data['invoice_type'],'show_signature')==1): ?>

          <div style="text-align:right; "class="me-2">Authorised Signatory</div>
