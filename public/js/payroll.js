$(document).ready(function(){
                    ...
                    $.each(checku, function( index, fvalue ) {
                        if ($.trim(fvalue)!='') {
                            if (isNaN(fvalue)==true) {
                                ...
                                const hasValue = Object.values(JSON.parse(json_fields)).includes($.trim(fvalue));
    ...
    $(document).on('click','.set_value',function(){
        ...
        $(this_btn).html('<span class="text-success"><b><i class="bx bx-check"></i></b> Save<span>');

        $('.manual_box'+box_id).addClass('aitsun-simple-input');
        $('.manual_box'+box_id).removeClass('aitsun-simple-input-disabled');
    ...
    $(document).on('click','.save_value',function(){
        ...
        $('.manual_box'+box_id).removeClass('aitsun-simple-input');
        $('.manual_box'+box_id).addClass('aitsun-simple-input-disabled'); 
        ...
        $('.manual_box'+box_id).each(function() {
            ...
            console.log(month_details+'---'+salary_id+'---'+field_id+'---'+employee_id) 
    ...
    $(document).on('click','.edit_payfield',function(){
        ...
        if (words.includes(ed_field_name)) {
          show_success_msg('error',' Failed!, <b>"'+ed_field_name+'"</b> is used in formula, please remove that before edit.');
        } else {
          show_success_msg('success','Saved!');
               ...
               $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        ...
                        // setting a timeout
                        $('#manual_punch').html('Saving...<i class="bx bx-loader bx-spin"></i> ');
