jQuery.fn.printIt = function(options){
    ...
    $(document).on('click','.aitsun-print',function(){
        ...
        $('body').append('<div id="pdf_print_loader" class="d-flex"><div class="pdf_print_box m-auto d-flex justify-content-center"><div class="d-flex my-auto"><i class="bx bx-loader bx-spin me-1"></i> <span id="pdddtext">Processing...</span></div></div></div>');
        var pdf_messages = ["Rendering content...", "Ready to print"]; // Add your messages here
        var pdf_index = 0;
        var intervalId;

        intervalId = setInterval(function() {
            $('#pdddtext').html(pdf_messages[pdf_index]);
             pdf_index++;

            if (pdf_index >= pdf_messages.length) {
