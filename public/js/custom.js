...
$(document).on('click','.big_data_export',function(){
...
$(document).on('click','.aitsun_table_print',function(){
	var table_selector=$(this).data('table');
	var export_to=$(this).data('type');
	var filename=$(this).data('filename');
...
$(document).on('click','.aitsun_table_quick_search',function(){
	var table_selector=$(this).data('table');
	...
	if (thead.length>0) {
		...
		$(table_selector+' thead tr th').each(function() {
		  ...
		  search_row+='<input type="text" class="row_search" placeholder="Search '+$.trim($(this).text())+'">';
			search_row+='</th>';
		});
...
$(document).on('keyup','.aitsun_table .filters input',function(e){
	/* Ignore tab key */
  ...
  $panel = $input.parents('.aitsun_table'),
...
$(document).on('click','.btnCancel',function(){
	...
	$(document).on('click','.pdf_open',function(){
		...
		$('#pdf_show').html('<iframe src="'+href+'/'+last_height+'" class="erp_iframe" id="erp_iframe"></iframe>');
	...
	$(document).on('click','.student_fee_data',function(){
		...
		$('#pdf_show').html('<iframe src="'+href+'#toolbar=0&navpanes=0&scrollbar=0" class="erp_iframe" id="erp_iframe"></iframe>');

		$('#do_btn').html('<a data-url="'+href+'" class="btn btn-back-dark me-2 aitsun-print">Print</a> <a href="'+download_href+'" class="btn btn-back me-4">Download</a>');
   ...
   $(document).on('change', '.save_payment_status', function(){
      ...
      Swal.fire({
        ...
        $.ajax({
        	 url: base_url()+'/aitsun_keys/payment_status/'+getid,
            type:'POST',
            data:{
              ...
              success: function(response) {
                  if ($.trim(response)==1) {
                  	...
                  	$('.add_cls-'+p_element+'-'+product_id).addClass('is-valid'); 
		            		setTimeout(function(){
		            			$('.add_cls-'+p_element+'-'+product_id).removeClass('is-valid');
		            		},2000)
                     ...
                     $('.add_cls-'+p_element+'-'+product_id).addClass('is-invalid'); 
		            		setTimeout(function(){
		            			$('.add_cls-'+p_element+'-'+product_id).removeClass('is-invalid');
		            		},2000)
                  }
              }
          });
...
$(document).on('submit','.on_submit_loader',function(){
	...
	$(this_button).html('<div class="spinner-grow spinner-grow-sm" role="status"> <span class="visually-hidden">Saving...</span></div>');
...
$(document).on('click','.aitsun_table_export[data-type="excel"],.aitsun_table_export[data-type="csv"],.aitsun_table_export[data-type="pdf"]',function(){
	...
	$(document).on('click','.href_long_loader',function(){
		var this_content=$(this).html();
		$('.page-content').html('<div class="apanel_loader2"><div><div class="spinner-loading"></div><div class="ddddddta"><div class="preloader"><div class="preloader_inner">100%</div></div></div><h3 class="load_h3">Please do not press back or close the app</h3></div><div class="page"></div></div>');
			loader();
	});
  ...
  function loader(_success) {
	...
	$(document).on('click','#find_btn',function () { //user clicks button
    if ("geolocation" in navigator){ //check geolocation available 
      //try to get user current location using getCurrentPosition() method
      if (navigator.geolocation) {
         navigator.geolocation.getCurrentPosition(function(position){ 
          $("#cmy_location").val(position.coords.latitude+","+ position.coords.longitude);
          $('#find_btn').html('<span class="text-success"><i class="fa fa-check"></i> Done</span>');
        });
       }else{
        $('#find_btn').html('<span class="text-danger"><i class="fa fa-check"></i> failed</span>');
       }
	...
	$(document).on('click','#add_customer',function() {
		...
		var gstinformat = new RegExp('^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]1}[1-9A-Z]{1}Z[0-9A-Z]{1}$');
			...
			if(name=='') {
				$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Please enter name</div>');
			}else if(name.length<4){
				$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Atleast 4 characters required</div>');
			}else{
	          ...
	          $("#gst_input").focus(); 
				...
				if (checkbox == true){
					if(area==''){
						$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Please Enter area</div>');
					}else if (company=='') {
						$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Please enter Company Name</div>');
					}else if (contacttype=='') {
						$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Please select Contact type</div>');
					}
					else if (designation=='') {
						$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Please select Designation</div>');
					}
					else{
						$('#error_mes').html('');
	...
	$(document).on('click','.click_inventory',function(){
		...
		if (is_crm==1) {
			...
			$.ajax({
          url: base_url()+"crm/get_against_data?redirect_url="+redirect_url+"&from_stage="+from_stage,
          success:function(response) {
	...
	$(document).on('click','.click_inventory_from_renew',function(){
		...
		if (is_crm==1) {
			...
			$.ajax({
          url: base_url()+"crm/get_against_data_from_renew?redirect_url="+redirect_url+"&from_stage="+from_stage+"&from_renew="+from_renew,
          success:function(response) {
            $('#display_against_crm_data').html(response);
          },
          error:function(){
	...
	$(document).on('click','.copy_me',function(){
		...
		var latext=newtext.replace('<span class="copiedsec">- <i class="bx bx-check-circle">Copied!</i></span>','');
		navigator.clipboard.writeText($.trim(latext));
		$(this).css("color","#bbbec1");
		$(this).find('.copiedsec').html('- <i class="bx bx-check-circle">Copied!</i>');
	});
	...
	$(document).on('input','.update_rate',function(){
		...
		if (rate=='' || rate==0) {
			...
			$.ajax({
	            url: base_url()+'/product_scrapper/update_rate?rate='+rate+'&cid='+cid,
	            success:function(response) {
	            	$('#savedcur').html('Saved!');
	            	setTimeout(function(){
	            		$('#savedcur').html('');
	...
	$(document).on('input','.update_profit',function(){
		...
		if (rate=='' || rate==0) {
			...
			$.ajax({
	            url: base_url()+'/product_scrapper/update_profit?rate='+rate+'&cid='+cid,
	            success:function(response) {
	            	$('#savedcur').html('Saved!');
	            	setTimeout(function(){
	            		$('#savedcur').html('');
		...
		$.ajax({
	            ...
	            success:function(result) {
	            	...
	            	if ($.trim(result)==1) {
	            		...
	            		$(sync_to_google_shop).html('<i class="bx bx-check-double text-white"></i>');
	            	}else{
	            		$(sync_to_google_shop).html('<i class="lni lni-google text-google"></i>');
	            ...
	            success:function(result) {
	            	...
	            	if ($.trim(result)==1) {
		            	...
		            	if (ip==len) {
		            		...
		            		$('#synmes').html('<div class="col-md-12"><div class="card" ><div class="card-body">Please wait few minutes... '+ip+' items completed</div></div></div>');
		            	}
	            	}else{
...
$(document).on('click','.sync_all_to_branch',function(){
		...
		Swal.fire({
		  ...
		  if (result.isConfirmed) {
		    ...
		    $(".sync_to_google_shop").each(function(index) {
		    	...
		    	if (idd==0 || idd=='') {idd='';}
		    	if (unit==0 || unit=='') {unit='no_val';}
		    	if (brand==0 || brand=='') {brand='no_val';}
		    	if (category==0 || category=='') {category='no_val';}
		    	if (sub_category==0 || sub_category=='') {sub_category='no_val';}
		    	if (sec_category==0 || sec_category=='') {sec_category='no_val';}
          ...
          var csrfHash = $('#csrf_token').val(); // CSRF hash
	        ...
	        $.ajax({
						type: "POST", 
	          url: base_url()+'/import_and_export/sync_products_to_branch/'+branchid+'/'+idd+'/'+unit+'/'+brand+'/'+category+'/'+sub_category+'/'+sec_category,
	           data:{
							[csrfName]: csrfHash
						},
						beforeSend:function(){  
	          	...
	          	$('#synmes').html('<div class="col-md-12"><div class="card" ><div class="card-body d-md-flex"><span class="my-auto">Please wait few minutes...</span><div class="spinner-border spinner-border-sm" role="status"> <span class="visually-hidden">Loading...</span></div><span class="my-auto">, please dont close/refresh the page until complete.</span></div></div></div>');
		        },
	          success:function(response) {
	          	$('#synmes').html('');
	          	$('#synmes').html('<div class="col-md-12"><div class="card" ><div class="card-body d-md-flex"><span class="my-auto text-success">Completed</span></div></div></div>');
		        },
		         error:function(){
		          alert("error");
		        }
      ...
      Swal.fire({
			  ...
			  if (result.isConfirmed) {
			    ...
			    if ($('#view_type').val()=='sales'){
		      	window.location.href = base_url()+"invoices/convert_to_sale_order/"+val;
		      }else{
		      	window.location.href = base_url()+"purchases/convert_to_purchase_order/"+val;
		      }  
	...
	$(document).on('click','#add_multiple_order_to_delivery_note',function(){
        ...
        Swal.fire({
				  ...
				  if (result.isConfirmed) {
				     ...
				     if ($('#view_type').val()=='sales'){
			        	window.location.href = base_url()+"invoices/convert_to_sale_delivery_note/"+val;
			        }else{
			        	window.location.href = base_url()+"purchases/convert_to_purchase_delivery_note/"+val;
			        } 
        ...
        $('input[name="pol_is_sdn[]"]:checked').each(function(i){
         ...
         Swal.fire({
		        ...
		        if ($('#view_type').val()=='sales'){
		        	...
		        	window.location.href = base_url()+"purchases/convert_to_purchase/"+val;
		        }
	...
	$(document).on('click','.delete,.delete_gp_head, .delete_work_category,.delete_enquiries,.thumbdel,.remove_sec_add_img,.remove_menu_add_img,.delete_track_status,.delete_order,.delete_home_slider_img,.remove_brand_img,.remove_sec_cat_img,.remove_sub_cat_img,.remove_cat_img,.delete_region,.delete_post,.delete,.product_delete,.delete_product_cat,.delete_product_unit,.delete_product_subcat,.delete_product_seccat,.delete_product_brand,.delete_tax_type,.delete_tmber,.pro_empty_all,.pro_delete,.purchase_delete_per,.inv_empty_all,.invoice_restore,.invoice_delete_per,.clear_20_logs,.clear_all_logs,.delete_payment,.delete_ex_payment',function(){
		...
		Swal.fire({
		  ...
		  if (result.isConfirmed) {  
            ...
            success: function(del_res) {  
              if (del_res==1) {
                  show_success_msg('success','Deleted!');
                  setTimeout(function(){
                  	location.reload();
                  },500);
              }else{
	...
	$(document).on('submit','form',function(){
	   $(this).find('.spinner_btn').html('<div class="spinner-grow" role="status"> <span class="visually-hidden">Loading...</span></div>');
	});
	...
	$(document).on('change','.parc',function(){
        ...
        var elem=$(this).data('proid'); 
        	...
        	if (unit!='None' && sub_unit!='None' && unit!='' && sub_unit!='' && unit!=sub_unit) {
			 ...
			 if (subunit!='') {
		      ...
		      var csrfHash = $('#csrf_token').val(); // CSRF hash
					...
					$.ajax({
			            ...
			            contentType: false,
			            beforeSend: function() {
			                $('#edit_save_product').html('<div class="spinner-grow" role="status"> <span class="visually-hidden">Loading...</span></div>');
			            },
			            success: function(result) {
...
$(document).on('change','#featured_image',function () {
    ...
    var csrfHash = $('#csrf_token').val(); // CSRF hash
			...
			if (scrap_url!='') {
				$.ajax({
			        url: base_url()+"product_scrapper/?siteid="+siteid+"&scrap_url="+scrap_url,
			        cache: false,    
			        data:{
			       		scrap_url:scrap_url,
			       		siteid:siteid,
			        ...
			        beforeSend:function(){  
		            	$('#scrap_product').html('Scrapping <i class="bx bx-loader bx-spin ms-2 my-auto" style="font-size: 17px;"></i>');
			        },
			        success: function(response){
						...
						if (data.brand!='') {
							$('#brand option[value='+data.brand+']').attr('selected','selected');
						}
            ...
            success:function(response) {
              if ($.trim(response)!=0) {
                $('#subunit_name'+elem).val('');
                 $('#subunit_container_product'+elem).toggle();
                $('#sub_unit'+elem).append('<option value="'+response+'" selected>'+subunit_name+'</option>');
              }else{
              	$('#subunits').html('<span class="text-danger" style="font-size: 12px;">Select parent unit</span>');
              }
           },
           error:function(){
            alert("error");
...
$('.addd_seccate').prop('disabled', true);
    ...
    if ($(this).prop("checked")) {
        ...
        $.ajax({
            ...
            success:function(result){
                if (statusname=='1') {
                    $('#'+getid+'rec_status').html(' <span class="px-2 py-1 rounded rec_status" style="font-size:12px;color: green;" >Received</span>');
                }else{
                    $('#'+getid+'rec_status').html(' <span class="px-2 py-1 rounded rec_status" style="font-size:12px;color: red;" >Not Received</span>');
                }
    ...
    if ($(this).prop("checked")) {
        ...
        $.ajax({
            ...
            success:function(result){
                if (statusname=='1') {
                    $('#'+getid+'rec_status').html(' <span class="px-2 py-1 rounded rec_status" style="font-size:12px;color: green;" >Scan copy received</span>');
                }else{
                    $('#'+getid+'rec_status').html(' <span class="px-2 py-1 rounded rec_status" style="font-size:12px;color: red;" >Scan copy not received</span>');
                }
  ...
  var row=1;
	          ...
	          $.ajax({
	            ...
	            success:function(response) {
	              ...
	              if ($.trim(response)!=0) {
	                $('#ex_cate_name').val('');
	                 $('#expense_container_cate').toggle();
	                $('#account_name').append('<option value="'+response+'" selected>'+ex_cate_name+'</option>');
	              }
	           },
	           error:function(){
	            alert("error");
	...
	$(document).on('click','.restore_doc_renew,.pro_restore,.pro_restore_all,.invoice_restore,.inv_restore_all,.purchase_restore',function(){
...
$(document).on('click','.cancel_renew',function(){
  ...
  $(document).on('click','.product_reject',function(){
		...
		if (reason=='') {
				$('#error_mes'+prorqid).html('<p class="text-danger mb-0">Please enter reason</p>');
...
$(document).on('click','#dnbox_close',function(){
  ...
  function display_users(){
		$.ajax({
			url:'https://aitsun.in/users/display_users',
			crossDomain: true,
			headers: {  'Access-Control-Allow-Origin': 'https://aitsun.in/' },
			success:function(data){
...
$(document).on('click','.delete_ctech',function(){
	...
	$(document).on('click','#add_ctechoman',function() {
	  ...
	  $('#error_mes').html('');
		...
		if (fullname=='') {
			...
			$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Atleast 3 chars required</div>');
		}else if (email=='') {
			$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Please enter your email</div>');
		}else if (!emailReg.test(email)) {
			$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Please enter valid email</div>');
		}else if (password=='') {
			$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Please enter password</div>');
		}
		else{
			$.ajax({
	...
	$(document).on('click','#add_aitsun_user',function() {
	  ...
	  $('#error_mes').html('');
		...
		if (fname=='') {
			...
			$.ajax({
				url:'http://localhost/aitsun.in/users/save_user',
				type:'POST',
			  crossDomain: true,
			  headers: {  'Access-Control-Allow-Origin': 'http://The web site allowed to access' },
				data: form_data,
...
$(document).on('click','.edit_ctechoman',function() {
	  ...
	  $('#error_mess'+ctechid).html('');
		...
		if (name=='') {
			$('#error_mess'+ctechid).html('<div class="alert alert-danger mb-3 mb-3">Please enter name</div>');
		}else if(name.length<3){
			$('#error_mess'+ctechid).html('<div class="alert alert-danger mb-3 mb-3">Atleast 3 chars required</div>');
		}else if (uemail=='') {
			$('#error_mess'+ctechid).html('<div class="alert alert-danger mb-3 mb-3">Please enter your email</div>');
		}else if (!emailReg.test(uemail)) {
			$('#error_mess'+ctechid).html('<div class="alert alert-danger mb-3 mb-3">Please enter valid email</div>');
		}
		else{
			$.ajax({
...
$(document).on('click','.edit_users',function() {
	  ...
	  $('#error_mes'+userid).html('');
		...
		if (firstname=='') {
			$('#error_mes'+userid).html('<div class="alert alert-danger mb-3 mb-3">Please enter name</div>');
		}else if(firstname.length<3){
			$('#error_mes'+userid).html('<div class="alert alert-danger mb-3 mb-3">Atleast 3 chars required</div>');
		}else if (user_email=='') {
			$('#error_mes'+userid).html('<div class="alert alert-danger mb-3 mb-3">Please enter your email</div>');
		}else if (!emailReg.test(user_email)) {
			$('#error_mes'+userid).html('<div class="alert alert-danger mb-3 mb-3">Please enter valid email</div>');
		}
		else{
			$.ajax({
...
function display_asms(){
		$.ajax({
			url:'https://aitsun.net/asms/users/display_users',
			crossDomain: true,
			headers: {  'Access-Control-Allow-Origin': 'https://aitsun.net/asms/' },
			success:function(data){
...
$(document).on('click','#add_asms_user',function() {
	  ...
	  $('#error_mes').html('');
		...
		if (username=='') {
			...
			$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Please enter your phone number</div>');
		}else if (max=='') {
			$('#error_mes').html('<div class="alert alert-danger mb-3 mb-3">Please enter your max</div>');
		}
...
$(document).on('click','.edit_asms_users',function() {
	  ...
	  $('#error_mes'+asmsid).html('');
		...
		if (fullname=='') {
			$('#error_mes'+asmsid).html('<div class="alert alert-danger mb-3 mb-3">Please enter name</div>');
		}else if(fullname.length<3){
			$('#error_mes'+asmsid).html('<div class="alert alert-danger mb-3 mb-3">Atleast 3 chars required</div>');
		}else if (asms_email=='') {
			$('#error_mes'+asmsid).html('<div class="alert alert-danger mb-3 mb-3">Please enter your email</div>');
		}else if (!emailReg.test(asms_email)) {
			$('#error_mes'+asmsid).html('<div class="alert alert-danger mb-3 mb-3">Please enter valid email</div>');
		}else if (asms_phone=='') {
			$('#error_mes'+asmsid).html('<div class="alert alert-danger mb-3 mb-3">Please enter your phone number</div>');
		}else if (asms_max=='') {
			$('#error_mes'+asmsid).html('<div class="alert alert-danger mb-3 mb-3">Please enter your max</div>');
		}
		else{
			$.ajax({
...
$(document).on('input','#wholesearch',function(e){
  ...
  if (checkedNum<1) {
      $('#er_msg').html('<div class="alert alert-danger p-2 ">Please Select atleast 1 student</div>');
      setTimeout(function() {
           $('#eccr').fadeOut();
      }, 1000); 
  }else{
     ...
     if (message=='') {
            $('#er_msg').html('<div class="alert alert-danger p-2">Please Enter message!</div>');
          }else{
         ...
         $.ajax({
              ...
              cache: false,
              beforeSend: function() {
                  $('#send_bulk').html('Sending p-2 <i class="anticon anticon-loading d-inline-block"></i> ');
                  $('#er_msg').html('<div class="alert alert-secondary p-2">Please wait & dont reload, it will takes few seconds</div>');
              },
              success: function(response) {
                  $('#send_bulk').html('<i class="anticon anticon-message mr-1"></i>Send SMS');
                  $('#bulkform')[0].reset();
  ...
  $(document).on('click','.add_fields_input',function(){
  	...
  	$('.field_select_container'+blockid).append('<input type="text" class="new_field_input" class="" id="field_inp'+blockid+'" data-blockid="'+blockid+'">');
  ...
  $(document).on('change','.field_select',function(){
  	...
  	if (this_val!='') {
  		$('.efb'+blockid).remove();
  		$('.field_select_container'+blockid).append('<a class="ml-2 my-auto btn btn-sm btn-codeview edit_fields_btn efb'+blockid+'" id="edit_fields_btn'+blockid+'" data-blockid="'+blockid+'"><i class="bx bx-edit m-auto"></i></a>');
  	}else{
  		$('.efb'+blockid).remove();
  	} 
  })
  ...
  $(document).on('click','.edit_fields_btn',function(){
		...
		$('.field_select_container'+blockid).append('<input type="text" class="edit_field_input efi'+blockid+'" class="" id="edit_field_inp'+blockid+'" data-blockid="'+blockid+'"><a class="ml-2 my-auto btn btn-sm update_fields_input" id="update_add_field'+blockid+'" data-blockid="'+blockid+'"><i class="bx bx-check m-auto"></i></a>');
		$('#edit_field_inp'+blockid).focus().val('').val(select_val);
	})
  	...
  	if (this_val.length>0) {
          ...
          success: function(response) {  
          	if ($.trim(response)!=0) {
          		...
          		var exists = $("#field_select"+blockid+" option")
               .filter(function (i, o) { return o.value === this_val; })
               .length > 0;
          		...
          		if (exists) {
          			...
          			$('#add_field_items').append('<option value="'+this_val+'" data-fid="'+fid+'">'+this_val+'</option>');
	          		$('#field_select'+blockid).append('<option value="'+this_val+'" data-fid="'+fid+'" selected>'+this_val+'</option>');
	          		$('#field_select'+blockid).val(this_val);
								$('#field_select'+blockid).trigger('change');
          		}  
	  	...
	  	$(thisinput).blur();
			...
			if (e.type=='focusout') {
				...
				if(regex.test(barcode_number) == true || barcode_number.length>20) {
				    $("#bar_inp"+product_id).val($("#bar_inp"+product_id).val().replace(/[^a-z0-9]/gi, ''));
				}else{
					$.ajax({ 
		        ...
		        success: function(data) {
		            ...
		            if ($.trim(data)==1) {
		            		...
		            		if (barcode_number!='' && barcode_number!=0) {
		            			...
		            			var new_src=base_url()+'/generate_barcode?text='+barcode_number+'&size=80&SizeFactor=4&print=true';
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if ($.trim(data)==1) {
                    		...
                    		$("#bar_box"+product_id).addClass('d-none');
                    		var new_src=base_url()+'/generate_barcode?text=&size=80&SizeFactor=4&print=true';
...
$(document).on('click','#save_staff_data',function(){
    ...
    if ($.trim(staff_name)=='') {
     $('#er_msg').html('<span class="text-danger">Employee name required!</span>');
      ...
      $('#er_msg').html('<span class="text-danger">Email not valid!</span>' );
        ...
        $('#er_msg').html('<span class="text-danger">Contact number required!</span>');
    }else if($.trim(designation)==''){
        $('#er_msg').html('<span class="text-danger">Designation required!</span>');
    }else if($.trim(shifts)==''){
        $('#er_msg').html('<span class="text-danger">Work shift required!</span>');
    }else if($.trim(staff_code)==''){
        $('#er_msg').html('<span class="text-danger">Employee code required!</span>');
    }else  
	   			{ 
				...
				$.ajax({
					...
					success:function(response){
						if (response==1) {
								$('#er_msg').html('<span class="text-success"><i class="bx bxs-check-circle me-1"></i>Saved!</span>');
								$('#add_staff_form')[0].reset();
						}else if(response==3){
								$('#er_msg').html('<span class="text-danger"><i class="bx bxs-error me-1"></i>Employee code already exist!</span>');
						}else if(response==2){
								$('#er_msg').html('<span class="text-danger"><i class="bx bxs-error me-1"></i>Email already exist!</span>');
						}else{
							$('#er_msg').html('<span class="text-danger"><i class="bx bxs-error me-1"></i>Failed to save!</span>');
						}
...
$(document).on('click','.edit_staff_data',function(){
    ...
    if ($.trim(staff_name)=='') {
     $('#er_msg'+employee_id).html('<span class="text-danger">Employee name required!</span>');
    }else if($.trim(staff_email)==''){
        $('#er_msg'+employee_id).html('<span class="text-danger">Email required!</span>');
    }else if(!emailReg.test(staff_email)) {
      $('#er_msg'+employee_id).html('<span class="text-danger">Email not valid!</span>' );
    }else if($.trim(phone_number)==''){
        $('#er_msg'+employee_id).html('<span class="text-danger">Contact number required!</span>');
    }else if($.trim(designation)==''){
        $('#er_msg'+employee_id).html('<span class="text-danger">Designation required!</span>');
    }else if($.trim(staff_code)==''){
        $('#er_msg'+employee_id).html('<span class="text-danger">Employee code required!</span>');
    }else  
	   			{ 
...
$(document).on('click','.generate_barcode',function(){
	...
	$(document).on('click','.save_notes',function(){
      ...
      if (text_note!='') {
        ...
        $.ajax({
          ...
          success:function(response) {
          	if ($.trim(response)==1) {
          		$('#appendable_section'+getid).append('<div class="position-relative"><div class="note_arrow" ></div><span class="ms-3 notes_text">'+text_note+'</span><span style="font-size: 11px;color: #a7a2a2;">'+time+'</span></div>');
              $('#text_note'+getid).val('')
            }  
          },
          error:function(){
  ...
  $(document).on('click','.add_parties_cat',function(){
            ...
            success:function(response) {
              if ($.trim(response)!=0) {
                ...
                $('#parties_cat_name').val('');
                 $('#cat_container_parties').toggle();
                $('#part_category').append('<option value="'+response+'" selected>'+parties_cat_name+'</option>');        
              }
           },
           error:function(){
              alert("error");
    ...
    $(document).on('click','.export_excel', function () {
            ...
            $.ajax({
                url:  base_url()+"export_excel", 
                type: 'POST',
                data:{
									[csrfName]: csrfHash
								},
								beforeSend:function(){
									show_success_msg('success','','Currently processing your download. We will notify you once its ready!');
	        			},
                success: function (response) {
                     if (response=='1') {
				              	show_success_msg('success','','Downloaded!');
				              }else{
				                show_success_msg('error','','Failed!');  
				             }
                }
    ...
    //                 });
    //             } else {
    //                 // File is not ready yet, you can perform other actions if needed
    //             }
    //         },
