...
$(document).on('input click keypress','#gst_input',function(){
	...
	$(document).on('append input paste','#typeandsearch,#product_code',function(){
		...
		$('#loadbox_name').html('');
		var input=$(this).data('input');
		var view_type=$("#voucher_voucher_type").val();
	...
	$(document).on('click','.item_box',function(){
		...
		var old_paid_amount=$(this).data('old_paid_amount');
                 ...
                 '<div class="mt-1">'+
                  '<div class="my-auto">'+
                    '<label style="opacity: 1;">Reference No.</label>'+
                      ...
                      '<input type="text" step="any" class="w-50 mb-0 form-control form-control-sm"  name="reference_no[]" id="reference_id<?= $row; ?>">'+
                  '</div>'+
                '</div>'+
