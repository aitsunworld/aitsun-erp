$(document).ready(function(){
    ...
    $(document).on('keydown','input[type="number"]',function(event) {
        ...
        if ($(this).hasClass('allow_minus')) {
             
            var keysToPrevent = ['+','*','/','e','E'];
        } else {
           var keysToPrevent = ['-', '+','*','/','e','E'];
    ...
    $(document).on('change','.is_pos',function(){
        ...
        $.ajax({
            ...
            success: function() {
                ...
                show_success_msg('success','Item added to POS items!')
...
$(document).on('click','.submit_invoice_form',function(){
        ...
        if (st>0) {
            if (pt!='') {
                ...
                $('#mess'+vehicleid).html('<span class="my-2">Select payment type!</div>');
            }

        }else{
            $('#mess'+vehicleid).html('<span class="my-2">Enter amount greater than 1!</div>');
        }
    }else{
        $('#mess'+vehicleid).html('<span class="my-2">Please select expense</div>');
    }
        ...
        $(document).on('click','#submit_challan',function(e){
            ...
            var due_amount=$('#due_amount').val();
              ...
              if (parseFloat(total_cash)>parseFloat(grand_total_after_discount)) {
                  ...
                  $('#mess').html('<span class="my-2 font-14">Cannot updated the challan with lower amount than current payment made. <a href="'+base_url()+'fees_and_payments/payments/'+inid+'">Delete vouchers</a></span>');
    ...
    function cinv(inid){
      $.ajax({
          type: "POST", 
                ...
                beforeSend:function(){  
                  $('#submit_challan').html('<span class="spinner-grow spinner-grow-sm mr-1" role="status" aria-hidden="true"></span> Loading...');  
               },
                success:function(response) {
                // alert(response);
                  $('#submit_challan').prop('disabled', false);
...
$(document).ready(function(){
    $('.check-mark').on('click', function(){
        var inputValue = $(this).closest('.dropdown_css').find('.form_input').val();
        var groupHeadId = $(this).data('group-head-id');
        var spantextid = $(this).data('spanid');
        ...
        $.ajax({
            type: 'POST',
            url: base_url()+'/home/get_adjust_payment/' + groupHeadId,
            data: {
...
$(document).on('click','.save_product_import', function(){ 
    var this_btn=$(this); 
    ...
    var title=$.trim($('#product_name'+element_id).val());
    
 
    slugg = title.replace(/\./g, "");
    var slugtext = slugg.split(" ").join("-").split("?").join("-").split("=").join("-").split("&").join("-").split("#").join("");
    var slug=slugtext;

    var unit=$.trim($('#unit_res'+element_id).val());
    var sub_unit=$.trim($('#sub_unit'+element_id).val());
    var conversion_unit_rate=$.trim($('#conversion_unit_rate'+element_id).val());
    var category=$.trim($('#category'+element_id).val());
    var product_type='single';
    var sub_category=$.trim($('#sub_category'+element_id).val());
    var secondary_category=0;
    var discounted_price=0;
    var price=$.trim($('#price'+element_id).val());
    var purchased_price=$.trim($('#purchased_price'+element_id).val());
    var barcode=$.trim($('#barcode'+element_id).val());
    var tax=$.trim($('#tax'+element_id).val());
    var stock=$.trim($('#stock'+element_id).val());
    var at_price=$.trim($('#at_price'+element_id).val());
    var purchase_tax=$.trim($('#purchase_tax'+element_id).val());
    var sale_tax=$.trim($('#sale_tax'+element_id).val());
    var mrp=$.trim($('#mrp'+element_id).val());
    var purchase_margin=$.trim($('#purchase_margin'+element_id).val());
    var sale_margin=$.trim($('#sale_margin'+element_id).val());
    var description=$.trim($('#description'+element_id).val());
    var product_method=$.trim($('#product_method'+element_id).val());
    var keywords='';
    var product_code=$.trim($('#product_code'+element_id).val());
    var delivery_days=0;
    var ex_date='0000-00-00';
    var pro_in=$.trim($('#pro_in'+element_id).val());
    var batch_no=$.trim($('#batch_no'+element_id).val());
    var bin_location=$.trim($('#bin_location'+element_id).val());
    var scrapped_by=0;
    ...
    if (purchase_margin!='') {
        p_margin=purchase_margin;
    }

    if (sale_margin!='') {
        s_margin=sale_margin;
    } 
    ...
    $.ajax({
        ...
        success: function(response) {
            // alert(response)
            if ($.trim(response)==1) {
                show_success_msg('success','','Imported!');
                var tr_content='';
             
                tr_content+='<td class="text-center"><b><i class="bx bx-check text-success" style="font-size: 20px;"></i></b></td>'
                        +'<td colspan="25"><b>'+title+'</b> is imported successfully</td>'
                        

                $('#imported_tr'+element_id).html(tr_content);
                // $('#display_status'+element_id).addClass('text-success');
                // $('#display_status'+element_id).addClass('bx-check');
                // $('#display_status'+element_id).removeClass('text-danger');
                // $('#display_status'+element_id).removeClass('bx-error');
                // $(this_btn).prop("disabled", true);
            }else{
                show_failed_msg('error','','Product exist/Failed to import!');
...
$(document).on('click','.save_import', function(){ 
    ...
    var display_name=$.trim($('#display_name'+element_id).val());
    var email=$.trim($('#email'+element_id).val());
    var phone=$.trim($('#phone'+element_id).val());
    var gst_no=$.trim($('#gst_no'+element_id).val());
    var u_type=$.trim($('#u_type'+element_id).val());
    var address=$.trim($('#address'+element_id).val());

    var bs=$.trim($('#bs'+element_id).val());
    var value_of_opening=$.trim($('#value_of_opening'+element_id).val());
    var op_type=$.trim($('#op_type'+element_id).val()); 
    ...
    $.ajax({
        type: 'POST',
        url: base_url()+'/customers/save_customer',
        data: {  
            ...
            opening_balance:value_of_opening,
            credit_limit:0,
            website:'',
...
$(document).on('click','.student_other_data',function(){
       ...
       var student_id = $(this).data('student_id');
            ...
            $.ajax({
              ...
              beforeSend:function(){  
                        $('#hide_student_data'+student_id).html('<div class="text-center p-4"><span>Please wait....</span><i class="bx bx-loader bx-spin"></i></div>');
                    },
              success:function(response) {
    ...
    $(document).on('click','.promote_stu_data',function(){
       ...
       var student_id = $(this).data('student_id');
            ...
            $.ajax({
              ...
              beforeSend:function(){  
                        $('#promote_student_data'+student_id).html('<div class="text-center p-4"><span>Please wait....</span><i class="bx bx-loader bx-spin"></i></div>');
                    },
              success:function(response) {
    ...
    $(document).on('click','.edit_stu_data',function(){
       ...
       var student_id = $(this).data('student_id');
            ...
            $.ajax({
              ...
              beforeSend:function(){  
                        $('#edit_student_data'+student_id).html('<div class="text-center p-4"><span>Please wait....</span><i class="bx bx-loader bx-spin"></i></div>');
                    },
              success:function(response) {
      ...
      $.ajax({
        ...
        success:function(response) {
          if ($.trim(response)!=0) {
            $('#display_name').val('');
            $('#brand_container_product').toggle();
            $('#driver').append('<option value="'+response+'" selected>'+display_name+'</option>');
          }
...
$(document).on('click','.save_side_bar_click',function(){
    ...
    $(document).on('click','.pro_stock_data',function(){
       var product_id = $(this).data('product_id');
            ...
            $.ajax({
              ...
              beforeSend:function(){  
                        $('#show_adjust_stock'+product_id).html('<div class="text-center"><span>Please wait....</span><i class="bx bx-loader bx-spin"></i></div>');
                    },
              success:function(response) {
    ...
    $(document).on('click','.pro_data',function(){
       var product_id = $(this).data('product_id');
       
            $.ajax({
              url: base_url()+"products/get_product_details/"+product_id,
              beforeSend:function(){  
                        $('#show_product_details'+product_id).html('<div class="text-center"><span>Please wait....</span><i class="bx bx-loader bx-spin"></i></div>');
                    },
              success:function(response) {
                $('#show_product_details'+product_id).html(response);
              }
            });
    ...
    $(document).on('click','.update_all_bar_type',function(){
        ...
        $.ajax({
            ...
            beforeSend: function() {
                $('body').append('<div id="print_loader" class="d-flex"><div class="print_box m-auto d-flex justify-content-center"><div class="d-flex my-auto"><i class="bx bx-loader bx-spin me-1"></i> Generating barcode...</div></div></div>');
            },
            success: function(data) {
                $('#print_loader').remove();
...
$(document).on('change','.week_selector',function(){
    ...
    if (this_sel_val=='custom') {
        ...
        $(thispar).find(".custom_week_input").removeClass('d-none').addClass('d-block');
    }else{
        var thispar=$(this_selector).parents(".week_parent");
        $(thispar).find(".custom_week_input").removeClass('d-block').addClass('d-none');
    }
...
///////////////////////////////// Manufacture scripts start ///////////////////////////////////////////
    ...
    $(document).on('input','.product_selector_search',function(){
        ...
        if (searchvalue.length>2) {
            $.ajax({
              ...
              beforeSend:function(){
                $('#show_pro').html('<div class="w-100 text-center"><div class="spinner-grow text-danger" style="width: 3rem; height: 3rem;" role="status"> <span class="visually-hidden">Loading...</span></div></div>');
    ...
    $(document).on('click','.add_to_item_kit',function(){
        ...
        if($("#proo"+product_id).length)         // use this if you are using id to check
        {
      ...
      var finprice=$('#main_price'+product_id).val(); 
            ...
            if ($('#in_unit'+product_id).children('option:first-child').is(':selected')) {
            ...
            var response=' <li class="list-group-item mb-2 d-flex justify-content-between align-items-center cursor-pointer itemparent" id="proo'+product_id+'"><input type="hidden" name="main_price[]" id="main_price'+product_id+'" value="'+purchased_price+'"> <input type="hidden" name="conversion_unit_rate[]" value="'+proconversion_unit_rate+'"><input type="hidden" name="unit[]" value="'+unit+'"><input type="hidden" name="sub_unit[]" value="'+sub_unit+'"><input type="text" name="item_product_name[]" class="form-empty bg-0" value="'+product_name+'" style="margin-right: 15px;" readonly><input type="number" name="item_quantity[]" id="qtyite'+product_id+'" data-proid="'+product_id+'" data-price="'+price+'" class="form-light itemkit_qty_input aitsun-simple-input form-light" data-row="'+product_id+'" data-proconversion_unit_rate="'+proconversion_unit_rate+'" value="1" step="any" style="margin-right: 15px; width:100px;"><select class="in_unit_material form-light aitsun-simple-input" name="in_unit[]" data-row="'+product_id+'" data-proconversion_unit_rate="'+proconversion_unit_rate+'" id="in_unit'+product_id+'" style="margin-right: 15px;">'+in_unit_options+'</select><input type="text" name="item_amount[]" class="form-light no-btn  aitsun-simple-input form-light totalamot" value="'+price+'" id="amtite'+product_id+'" style="margin-right: 15px; width:150px;" readonly><input type="hidden" name="item_product_id[]" value="'+product_id+'"><input type="hidden" name="item_price[]" value="'+price+'"><input type="hidden" name="purchased_price[]"  value="'+purchased_price+'"><input type="hidden" name="item_selling_price[]"  value="'+price+'"><input type="hidden" name="item_product_desc[]"  value="'+description+'"><input type="hidden" name="item_tax[]"  value="'+tax+'"><button type="button" class="btn-dark btn btn-sm removeitemkit rounded-pill"><span class="rotate-45 d-block">+</span></button></li>';
...
$(document).on('input','.itemkit_qty_input',function(){
        ...
        if (proconversion_unit_rate>0) { 
            ...
            if ($('#in_unit'+rowno).children('option:first-child').is(':selected')) {
...
$(document).on('input change','#man_quantity_input,#man_unit_input',function(){
...
$(document).on("click",".add-more",function(){ 
      var html = '<tr class="after-add-more-tr"><td class="w-25"><input type="hidden" name="co_i_id[]" value="0"><input type="hidden" name="old_additional_cost[]"  value="0"><select name="additional_charges[]" class="aitsun-simple-input"><option value="Labour cost">Labour cost</option><option value="Electricity cost">Electricity cost</option><option value="Packaging charge">Packaging charge</option><option value="Logistic cost">Logistic cost</option><option value="Other charges">Other charges</option></select></td><td class="w-75"><input type="text" name="additional_details[]" class="aitsun-simple-input"></td><td><input type="number" name="additional_cost[]" style="width: 300px;" class="aitsun-simple-input cost_input text-end"></td><td class="change"><a class=" no_load btn btn-danger btn-sm remove text-white"><b>-</b></a></td></tr>'; 
...
function count_total_cost(){
    ...
    $(".cost_input").each(function() { 
        ...
        if (cost_val!='') {
            total_additional_cost=parseFloat(total_additional_cost)+parseFloat(cost_val);
        }    
    });
...
function total_manufacture_cost(){
    ...
    $(".man_cost_input").each(function() { 
        ...
        if (man_cost_val!='') {
            total_manufacture_cost=parseFloat(total_manufacture_cost)+parseFloat(man_cost_val);
        }  
        // alert(man_cost_val) 
    });
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                data: $('#manufacture_form').serialize(), 
                 beforeSend: function() { 
                    // setting a timeout
                    $('#manufacture_this').html('Manufacturing...<i class="bx bx-loader bx-spin"></i> '); 
                },
                success: function(result) {
                    if (result==1) {
...
///////////////////////////////// Manufacture scripts end ///////////////////////////////////////////
               ...
               $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        ...
                        // setting a timeout
                        $('#add_time_table').html('Saving...<i class="bx bx-loader bx-spin"></i> ');
    ...
    $.ajax({
        url:base_url()+'/fees_and_payments/get_all_fees_list/all/8/'+invoice_id,
        success:function(data){
 ...
 $(document).on('click','.click_to_add_fees',function(){
    ...
    if ($(".itemforcheck"+productid).length<1) {
        $('.search_fee_select').append('<div class="card_fees rounded-3 border mb-2 itemforcheck'+productid+'" id="fee_row'+productid+'">'+
        '<div class="d-flex justify-content-between ">'+
        '<div class="d-flex justify-content-between w-100 p-2">'+
        '<h6 class="my-auto font-14">'+product_name+'</h6>'+
        '<input type="hidden" name="product_name[]" value="'+product_name+'">'+
        '<input type="hidden" name="product_id[]" value="'+productid+'">'+
        '<input type="hidden" name="initem_id[]" value="0">'+
        '<input type="number" step="any" name="fee_item_price[]" class="form-control form-control-sm w-25 fees_item_price" value="'+product_price+'"></div>'+
        '<a class="btn btn-danger d-flex align-items-center fee_row_remove" id="'+productid+'"><span>X</span></a></div></div>');
    ...
    if (e.keyCode == 13) {
        ...
        if (search_text.length>0) {
            if (search_text.length>2) {
                ...
                $.ajax({
                    type: 'GET',
                    url: select_url+'/'+search_text+'/'+fees_limit+'/'+invoice_id, 
                    beforeSend: function() { 
                    },
                    success: function(response) {  
 ...
 $(document).on('keypress','#search_fees_data',function (e) {
    if (e.keyCode == 13) {
        var this_elem=$(this);
        var fees_limit=3;
        var search_text=$.trim($(this).val());
    ...
    $(document).on('click','#deleteallbtn',function(){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = base_url()+"/fees_and_payments/delete_invoice/"+ffedid+'/'+val; 
            }
        });
      ...
      var fees_id=$(this).data('fees_id');
        ...
        $.ajax({
            ...
            data: {
             ...
             beforeSend: function() {
                // setting a timeout
                $(thiss).html('Notifying <i class="bx bx-loader spin_me d-inline-block"></i> ');
    ...
    $(document).on('click','#sms_notify_student_fees',function(){
        ...
        $.ajax({
            ...
            contentType: false,
             beforeSend: function() {
                // setting a timeout
                $(thiss).html('Sending <i class="bx bx-loader spin_me d-inline-block"></i> ');
    ...
    $(document).on('change','#no_of_installments',function(){
        ...
        var emi_amount=0;
        emi_amount=(parseFloat(payable)/no_of_installments).toFixed(round_of_value());

        for (var i = 0; i < no_of_installments; i++) {
            var counts=parseInt(i)+1;
            calculated_details+='<tr><td>Installment '+counts+'</td><td>-</td><td>'+cursymb+' '+parseFloat(emi_amount).toFixed(round_of_value())+' <input type="hidden" value="'+parseFloat(emi_amount).toFixed(round_of_value())+'" name="installments[]"></td></tr>';
        }
     ...
     $(document).on('click','#save_payment',function(){ 
          ...
          if(payment_date==''){ 
              ...
              show_success_msg('error','Please enter an amount less than or equal to the total amount!'); 
              $('#save_payment').html('Save');
          }else{
            ...
            $('#save_payment').prop('disabled', true);
              ...
              $.ajax({  
                   ...
                   if ($.trim(data)=='morethandue') { 
                    show_success_msg('error','Enter amount less than or equal to due amount'); 
                   }else{
                      ...
                      $('#save_payment').html('Save');
                        ...
                        const iframe = document.getElementById('aitsun-embed');
                      iframe.srcdoc = '<!DOCTYPE html><div style="color: green; background:white; width: 100%;height: 96vh;display: flex;"><div style="margin:auto; font-family: system-ui;">Rendering Content...</div></div>';
...
$(document).on('click','.delete_challan_link',function(){
        ...
        Swal.fire({
              ...
              closeOnConfirm: true
                  ...
                  if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: base_url()+'/users/check_challan_transaction_exist/'+challan_id,
                        data:{
                            [csrfName]: csrfHash
                        },
                        success: function(res_data) { 
                            if ($.trim(res_data)=='exist') {
                                show_success_msg("error", "Please delete all transactions before delete", "error");
                            }else{
    ...
    $(document).on('click','.generate_invoices',function(){
            ...
            if (class_id_select!='') {
                ...
                Swal.fire({
                  ...
                  if (result.isConfirmed) {
                    ...
                    $.ajax({
                         type: 'POST',
                        url: base_url()+'fees_and_payments/generate_for_all_transport/'+fees_id,
                        data:{
                             [csrfName]: csrfHash
                        }, 
                        beforeSend: function() {
        ...
        $(document).on('click','.generate_invoice_for_student',function(){
            ...
            if (student_id!='') {
                $.ajax({
                    type: 'POST',
                    url: base_url()+'fees_and_payments/generate_invoices/'+fees_id+'/'+student_id,
                    data:{
                         [csrfName]: csrfHash
                    }, 
                    beforeSend: function() {
        ...
        $(document).on("click",".add-more-custom",function(){ 
              var html = '<tr class="after-add-more-custom-tr"><td><input type="text" name="custom_fees_name[]" class="form-control "></td><td><input type="number" name="custom_fees_amount[]" class="form-control text-end custom_fees_amount"></td><td class="change"><a class="btn btn-danger btn-sm no_load  remove-custom text-white"><b>-</b></a></td></tr>'; 
                ...
                if ($(".after-add-more-custom-tr").length<1) { 
                  show_success_msg('error','Please add atleast 1 item to challan!');
                    ...
                    if (proceed==true) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: base_url()+'fees_and_payments/generate_invoices_custom/'+fees_id+'/'+student_id,
                        ...
                        show_success_msg('error','Please add atleast 1 valid fees name & amount!');
                    }
    ...
    ///////////////// custom fees
        ...
        $(document).on('click','.generate_transport_invoice_for_student',function(){
            var fees_id=$(this).data('fees_id');
            ...
            if (student_id!='' && location_id!='') {
                $.ajax({
                    type: 'POST',
                    url: base_url()+'fees_and_payments/generate_invoices_for_transport/'+fees_id+'/'+student_id+'/'+location_id,
                    data:{
                         [csrfName]: csrfHash
                    }, 
                    beforeSend: function() {
                    ...
                    success: function(response) {
                        ...
                        if ($.trim(response)==1) {
                            ...
                            if ($.trim($('#select_profile_img').val())!='') {
                                ...
                                oFReader.readAsDataURL(document.getElementById("select_profile_img").files[0]);

                                oFReader.onload = function (oFREvent) {
                                    document.getElementById("ac_profile").src = oFREvent.target.result;
                                };
                            }
           ...
           if (valid == true) {
               $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#change_pass').html('Saving...<i class="bx bx-loader bx-spin"></i>');
                    ...
                    success: function(result) {
                        ...
                        if ($.trim(result)==1) {
                          $('#pass_form')[0].reset();
                          show_success_msg('success','Password changed successfully!','Saved!');
                        ...
                        success: function() {
                            ...
                            Swal.fire(
                              ...
                              'success'
                                ...
                                window.location.href = base_url()+'library-management/issuedbooks?page=1';
                            });
                        }
                    });
               }
                ...
                success: function() {
                    ...
                    Swal.fire(
                              ...
                              'success'
                                ...
                                window.location.href = base_url()+'library-management/returnedbooks?page=1';
                            });
                ...
                success: function() {
                    ...
                    Swal.fire(
                              ...
                              'success'
                                ...
                                window.location.href = base_url()+'library-management/bookcategory';
                            });
       ...
       if (valid == true) {
           ...
           $.ajax({
                ...
                contentType: false,
                 beforeSend: function() {
                    // setting a timeout
                    $('#add_category').html('Saving...<i class="bx bx-loader bx-spin"></i>');
       ...
       if (valid == true) {
           $.ajax({
                ...
                success: function(response) {
                    ...
                    if ($.trim(response)==1) {
                        $(techerbt).html('Save');
                        show_success_msg('success','Category updated successfully!','Saved!');
                    }else{
                        show_failed_msg('error','','Failed!'); 
                    }
          ...
          $.ajax({
            ...
            success:function(response) {
              if ($.trim(response)!=0) {
                $('#display_name').val('');
                $('#teacher_container').toggle();
                $('#teacher').append('<option value="'+response+'" selected>'+display_name+'</option>');
              }
        ...
        $('#add_subjects').prop('disabled', true);
           $.ajax({
                ...
                contentType: false,
                 beforeSend: function() {
                    // setting a timeout
                    $('#add_subjects').html('Saving...<i class="bx bx-loader bx-spin"></i>');
                ...
                success: function(response) {
                    if ($.trim(response)==1) {
                        ...
                        show_success_msg('success','Subject added successfully!','Saved!');
                    }else{
                        $('#add_subjects').html('Save');
                       show_failed_msg('error','','Failed!');
                    }
                    ...
                    success: function() {
                         ...
                         Swal.fire(
                              ...
                              'success'
                                ...
                                window.location.href = base_url()+'class-and-subjects/subjects';
                            });
...
$(document).on('click','#add_class',function(){
     ...
     $(document).on('click','.deletestudent',function(){
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'POST',
                    url: base_url()+'student-master/check_transaction_exist/'+student_id,
                    data:{
                        [csrfName]: csrfHash
                    },
                    success: function(res_data) { 
           ...
           $.ajax({
                ...
                success: function(result) {
                    ...
                    if ($.trim(result)=='big_file') {
                        $(studentbt).html('Save');
                        show_failed_msg('error','Profile size must be less than 500kb!','Failed!');
                        
                     }else if ($.trim(result)=='email_exist') {
                       show_failed_msg('error','Mobile number already exists!','Failed!');
                       $(studentbt).html('Save');
            ...
            $('#add_student_new').prop('disabled', true);
               $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#add_student_new').html('Saving...<i class="bx bx-loader bx-spin"></i>');
                         ...
                         if ($.trim(response)=='email_exist') {
                           ...
                           if (added_from=='organisation') {
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    type: 'GET',
                                    url: base_url()+'student-master/get_fees_list_of_student/'+response,
...
$(document).on('click','.generate_challan_for_student',function(){
        ...
        $.ajax({
            ...
            beforeSend: function() {
                ...
                $(this_btn).html('<div class="card bg-success"><div class="card-body text-white">Please wait until complete ...<i class="ml-2 bx bx-loader spin_me d-inline-block"></i> </div> </div>');
            },
            success: function(response) {
    ...
    $(document).on('click','#do_filter_mess',function(){
        ...
        msg_std_to=20;
        msg_std_from=0;
        msg_std_load_data(msg_std_to,msg_std_from,strsearch,'html',str_serachclass,str_gender);
        msg_std_search=strsearch;
        msg_std_serachclass=str_serachclass;
        msg_std_gender=str_gender;

        $(window).scroll(function(){
        if($(window).scrollTop() + $(window).height() > $("#all_student_details").height() && msg_std_action == 'inactive')
        {
          msg_std_lazzy_loader(msg_std_to);
          msg_std_action = 'active';
          msg_std_from = msg_std_from + msg_std_to;
          setTimeout(function(){
            msg_std_load_data(msg_std_to,msg_std_from,msg_std_search,'append',msg_std_serachclass,msg_std_gender);
          }, 1000);
        }
      });
       ...
       if (valid == true) {
            ...
            $.ajax({
                ...
                contentType: false,
                 beforeSend: function() {
                    // setting a timeout
                    $('#add_sports').html('Saving...<i class="bx bx-loader bx-spin"></i>');
       ...
       if (valid == true) {
                ...
                $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#add_participant').html('Saving...<i class="bx bx-loader bx-spin"></i>');
                    },
                    success: function(response) {
                         ...
                         if ($.trim(response)=='sports_exist') {
                            ...
                            show_failed_msg('error','Sports category already exists!','Failed!');
                    ...
                    success: function() {
                        ...
                        Swal.fire(
                              ...
                              'success'
                                ...
                                window.location.href = base_url()+'school_activities/sports_participants?page=1';
                            });
           ...
           if (valid == true) {
                ...
                $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#add_sportsevent').html('Saving...<i class="bx bx-loader bx-spin"></i>');
                    ...
                    success: function() {
                        ...
                        Swal.fire(
                              ...
                              'success'
                                ...
                                window.location.href = base_url()+'school_activities/sports_events?page=1';
                            });
                    }
                    });
                ...
                if (reward_status!='') {
                    $.ajax({
                        ...
                        success: function(response) {
                            if ($.trim(response)==1) {
                                show_success_msg('success','','Saved!');
                                $('#rw_status'+studentid).text(htmlEntities($('#rw_val'+studentid).val()));
                            }else{
                                show_failed_msg('error','','Failed!');
                            }
                        }
   ...
   $(document).on('click','#add_activity',function(){
           ...
           if (valid == true) {
               ...
               $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#add_activity').html('Saving...<i class="bx bx-loader bx-spin"></i>');
                    ...
                    success: function(response) {
                        ...
                        if ($.trim(response)==1) {
                            ...
                            show_success_msg('success','Activity added successfully!','Saved!');
                        }else{
                            $('#add_activity').html('Save');
                           show_failed_msg('error','','Failed!');
                        }
           ...
           if (valid == true) {
               $.ajax({
                    ...
                    success: function(response) {
                       ...
                       if ($.trim(response)==1) {
                            $(techerbt).html('Save');
                            show_success_msg('success','Activities details updated!','Saved!');
                        }else{
                            $(techerbt).html('Save');
                           show_failed_msg('error','','Failed!');
                        } 
           ...
           $('#add_actparticipant').prop('disabled', true);
               $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#add_actparticipant').html('Saving...<i class="bx bx-loader bx-spin"></i>');
    ...
    $(document).on('click','.edit_actparticipants',function(){
            ...
            var techerbt=$(this);
            var form_data = new FormData($('#edit_actparticipant_form'+actpartcipid)[0]);
                ...
                success: function() {
                    ...
                    Swal.fire(
                              ...
                              'success'
                                ...
                                window.location.href = base_url()+'school_activities/eccc_participants?page=1';
                            });
    ...
    $(document).on('input','.analytics_eccc',function(){
            ...
            $('#add_ecccevent').prop('disabled', true);
                $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#add_ecccevent').html('Saving...<i class="bx bx-loader bx-spin"></i>');
                    ...
                    success: function() {
                       ...
                       Swal.fire(
                              ...
                              'success'
                                ...
                                window.location.href = base_url()+'school_activities/eccc_events?page=1';
                            });
     ...
     $(document).on('click','#backdrop_for_ent,.quick_menu_container',function(){ 
        ...
        if (due_amount>0) {
            if($('#fees_container').hasClass('d-none')){
                ...
                $.ajax({
                    ...
                    success: function(feeform) { 
                        $('#fee_show').html(feeform);
                        $('#fee_show').append('<div class="close_fees_container"><i class="anticon anticon-close"></i></div>');
                    }
                }); 
    ...
    $(document).on('click','.generate_new_challan',function(){
       ...
       var taypeee='<li class="btn d-block btn-info generate_standard_fee" data-std_id="'+std_id+'"><span>Standard</span></li><li class="btn d-block btn-secondary mt-2 generate_transport_fee" data-std_id="'+std_id+'"><span>Transport</span></li>';
    ...
    $(document).on('click','.tr_fees_list',function(){
        ...
        $.ajax({
            type: 'GET',
            url: base_url()+'users/get_all_transport_locations/'+std_id+'/'+fees_id,
            success: function(feedata) {
                $('#sug_ul').html(feedata);
            } 
        });
   ...
   $(document).on('click','.confirm_before_link',function(){
           ...
           if (valid == true) {
               ...
               $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#add_vehicle').html('Saving...<i class="bx bx-loader bx-spin"></i>');
        ...
        $('#add_std_location').prop('disabled', true);
           $.ajax({
                ...
                contentType: false,
                 beforeSend: function() {
                    // setting a timeout
                    $('#add_std_location').html('Saving...<i class="bx bx-loader bx-spin"></i>');
    ...
    $(document).on('input','.easy_stu_update',function(){
        ...
        var csrfHash = $('#csrf_token').val(); // CSRF hash
          ...
          $.ajax({
              type: 'POST',
              url: base_url()+'student-master/student_easyedit/'+student_id+'/'+classtableid,
              data: {
              ...
              success: function(response) {
                  if ($.trim(response)==1) {
                    $('.add_cls-'+p_element+'-'+student_id).addClass('aitsun-is-valid'); 
                            setTimeout(function(){
                                $('.add_cls-'+p_element+'-'+student_id).removeClass('aitsun-is-valid');
                            },2000)

                      // round_success_noti('Saved');
                  }else{
                     $('.add_cls-'+p_element+'-'+student_id).addClass('aitsun-is-invalid'); 
                            setTimeout(function(){
                                $('.add_cls-'+p_element+'-'+student_id).removeClass('aitsun-is-invalid');
                            },2000)
                  }
              }
          });
    ...
    $(document).on('click','.add_cal_event',function(){
        ...
        if (title!='' && start!='') {
            ...
            $('#er_mes'+tid).html('<span class="text-danger px-3 mb-3 d-block">Title & Start date is required</span>');
        } 
            ...
            $('#add_exam_cate').prop('disabled', true);
               ...
               $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#add_exam_cate').html('Saving...<i class="bx bx-loader bx-spin"></i>');
           ...
           if (valid == true) {
               ...
               $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#add_main_exam').html('Saving...<i class="bx bx-loader bx-spin"></i>');
            ...
            $('#add_main_exam_subject').prop('disabled', true);
               ...
               $.ajax({
                    ...
                    contentType: false,
                     beforeSend: function() {
                        // setting a timeout
                        $('#add_main_exam_subject').html('Saving...<i class="bx bx-loader bx-spin"></i>');
                    ...
                    success: function() {
                       ...
                       Swal.fire(
                          ...
                          'success'
                            ...
                            window.location.href = base_url()+'exams/main_exam_time_table/'+main_ex_id;
                        });
     ...
     $(document).on('click','.edit_main_exam_form',function(){
        ...
        var noticedid=$(this).data('id');
        var techerbt=$(this);
        var form_data = new FormData($('#edit_main_exam_subject_form'+noticedid)[0]);
     ...
     $(document).on('click','.publish_exam_via_sms',function(){
              ...
              Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: base_url()+'messaging/publish_exam_via_sms/'+exid+'?classid='+boxid,
                        data:{
                            [csrfName]: csrfHash
                        },
                        beforeSend: function() {
                            
                            $('#btttn'+boxid).html('Please wait...<i class="bx bx-loader bx-spin"></i>');
                        },
                        success: function(data) {
    ...
    $(document).on('click','.publish_exam_result_via_sms',function(){
          ...
          Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: base_url()+'messaging/publish_exam_result_via_sms/'+exid+'?classid='+boxid,
                        data:{
                            [csrfName]: csrfHash
                        },
                        beforeSend: function() {
                            
                            $('#btttnn'+boxid).html('Please wait...<i class="bx bx-loader bx-spin"></i>');
                        },
                        success: function(data) {
                             $('#errorbox'+boxid).html(data);
    ...
    $(document).on('click','.notify_exam_result',function(){
          ...
          Swal.fire({
                ...
                $.ajax({
                    type: 'POST',
                    url: base_url()+'exams/notify_main_exam_result/'+exid+'?classid='+boxid,
                    data:{
                        [csrfName]: csrfHash
                    },
                    beforeSend: function() {
    ...
    //add_notice
        $(document).on('click','#send_notice',function(){
           ...
           var strippedword=summerrich.replace(/(<([^>]+)>)/ig,"");
                ...
                if (final_richcontent!='') {
                    ...
                    $.ajax({
                        ...
                        contentType: false,
                         beforeSend: function() {
                            // setting a timeout
                            $('#send_notice').html('Saving  <i class="anticon anticon-loading d-inline-block"></i> ');
    ...
    $(document).on('change','.add_and_remove_fees',function () {
        ...
        $('#search_feestable_data .add_and_remove_fees').prop('disabled', true);
      ...
      $.ajax({
        ...
        success: function(data) {
            if ($.trim(data)==1) {
                show_success_msg('success','saved!');
                 $('#search_feestable_data .add_and_remove_fees').prop('disabled', false);
            ...
            $.ajax({
                ...
                success: function(cat_result) {
                  ...
                  if (cat_result!=0) {
                    ...
                    $('#parent_category').val('');
                    $('#new_category').val('');
                    $('#post_category').append('<option value="'+cat_result+'" selected>'+new_category+'</option>');
                    $('#post_category').focus();
                  } 
            ...
            $.ajax({
                ...
                success:function(response){ 
                    if ($.trim(response)==1) {
                        Swal.fire(
                          'Deleted!',
                          'Concession amount has been removed.',
                          'success'
                        ) 
                        location.reload(); 
                    }else{
